...
!window.FAM &&
  (function () {
    ...
    window.FAM = {
      ...
      config: {
        chat_page: "/f37-sagyetok",
        chat_permission: "member", //  peut etre [all] ou [member] ou [staff]
        main_title: "SagyeTok",
        ...
        ignore_announcements: true, // [false] pour voir les sujets d'annonces et de note, [true] pour les cacher
      },
      ...
      lang: {
        loading: "Chargement",
        idle: "SagyeTok est en veille.<br> Cliquez pour reprendre.",
        message_notif_singular: "$N nouveau message",
        message_notif_plural: "$N nouveau messages",
        new_messages: "Nouveaux Messages",

        new_topic: "Nouvelle discussion",
        start_topic: "Ouvrir une discussion",
        forum_name: "Dossiers",
        topic_name: "Discussions",

        msg_placeholder: "Envoyer un message",
        title_placeholder: "Titre de la discussion",
        load_older: "Voir les anciens messages",
        delete_message: "Voulez-vous supprimer ce message ?",

        topic_no_subject: "Vous devez indiquer un titre pour cette discussion.",
        topic_no_message: "Vous devez écrire un message avant d'ouvrir cette discussion.",
...
document.addEventListener('DOMContentLoaded', function() {
    function updatePlusButtonStyle() {
        // Select the container and the plus button
        var tabContainer = document.getElementById('FAM-tab-container');
        var plusButton = document.getElementById('FAM-tab-add');

        // Get the number of active tabs
        var activeTabs = document.querySelectorAll('#FAM-tabs .FAM-tab-active');

        if (activeTabs.length === 0) {
            // No active tabs, apply full border-radius
            plusButton.style.borderRadius = '5px';
        } else {
            // There are active tabs, apply right-side border-radius
            plusButton.style.borderRadius = '0 5px 5px 0';
        }
    }
    ...
    // Use MutationObserver to watch for changes in the tab container
    ...
    // Observe changes in the tab container
    observer.observe(document.getElementById('FAM-tabs'), { childList: true, subtree: true, attributes: true });
});